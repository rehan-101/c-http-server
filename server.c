...
void patch_func(socket_t fd, struct httpRequest *Request)
{
    ...
    send_json(fd, 200, "OK", json_response);
    if (!json_response)
    {
        json_response = strdup("\"message\" :\"did not get any response from the server on that specific uri\"");
        send_json(fd, 404, "Not Found", json_response);
    }
    close(fd);
    clean_things((void *)json_response, NULL);
}
...
void delete_func(socket_t fd, struct httpRequest *Request)
{
    ...
    {
        json_response = strdup("\"Message\" : \"This endpoint is not defined yet\"");
        send_json(fd, 404, "Not found", json_response);
    }
    close(fd);
    clean_things(json_response, NULL);
}
...
const char *handle_patch_uri(const char *uri, const char *body)
{
    ...
    json = strncmp(uri, "/users/", 7) == 0 ? (is_just_id(uri + 7) ? handle_patch_with_id(atoi(uri + 7), body) : strdup("\"message\" : \"format is not /users/{id}\"")) : strdup("\"message\" : \"endpoint is not defined yet\"");
    return json;
}