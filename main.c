...
{
    struct Server server = server_constructor(AF_INET, 8000, SOCK_STREAM, 0, 10, INADDR_ANY, get_func);
    struct httpRequest *REQUEST = parse_methods("GET / HTTP/1.1");
    if (REQUEST != NULL)
